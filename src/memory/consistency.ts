...
// `importance` is reserved too: on this CockroachDB store salience lives in
// `metadata.importance` (there is no dedicated column), so it is a RESOLVER signal
// (read by importanceOf), never a record attribute — two memories carrying
// different salience for one record must not read as a spurious contradiction.
const RESERVED_KEYS = new Set(["record", "refs", "importance"]);
