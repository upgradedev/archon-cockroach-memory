...
**Read this before opening the demo URL.** The CockroachDB Cloud Basic cluster
reached its Request Unit allowance and was disabled on 2026-08-02, so
`https://d2s5v0o0eg2aaw.cloudfront.net` currently serves the SPA and
`/api/health`, while `/api/proof`, `/api/audit` and `POST /api/recall` return
HTTP 500. Full detail is in [`docs/DEMO_URL.md`](./DEMO_URL.md) and the outage is
reviewed as a Reliability finding in
[`docs/WELL_ARCHITECTED_AGENTIC_REVIEW.md`](./WELL_ARCHITECTED_AGENTIC_REVIEW.md).
Rows below that depend on a live data plane say so in the How column rather than
linking to a URL that will disappoint you.
...
| The resolution loop cannot write to canonical memory | The runtime principal has `SELECT` only on `agent_memory` and no `INSERT`/`UPDATE`/`DELETE` on any relation. Mutation exists only inside two granted functions scoped to the sandbox tables. | [`src/db/schema.sql`](../src/db/schema.sql) lines 408-460 and 957-968 |
...
| Tests cover the runtime, not only the happy path | 43 test files under `tests/`, including health-probe timeout, rejection and never-settling cases, alarm routing, API stage proof, and row-level-security behaviour. CI runs them on every push. | [run 30927004090](https://github.com/upgradedev/archon-cockroach-memory/actions/runs/30927004090) at `25c8f443`; [`tests/`](../tests) |
...
| Answers are grounded in cited memories, with a numeric guard | The narrator cites every sentence, bounds assertable numbers to lexemes present in the cited memories, rewrites from scratch when a deterministic grounding check fails, and falls back to an extractive answer rather than inventing one. With no recall it returns a no-evidence trace instead of an answer. | [`src/agents/narrator.ts`](../src/agents/narrator.ts) lines 101-200 |
...
that some of these controls are proven, some are source-only, and one class of
them is currently failing. The distinction is drawn in every row.
...
| A recovery watchdog runs on a schedule and currently succeeds | Detects and repairs drift in the AWS foundation. | [run 30926605419](https://github.com/upgradedev/archon-cockroach-memory/actions/runs/30926605419) at `3fbba06c` |
...
**Known-failing controls, stated here rather than omitted.** Alarm notification
...
| Row-level security is used as an agent memory boundary | RLS enabled and forced on canonical memory and on all five sandbox tables, with paired permissive and restrictive policies so a permissive policy alone cannot widen access, and `NOBYPASSRLS` runtime roles. | [`src/db/schema.sql`](../src/db/schema.sql) lines 398-405, 981-1004 |
...
| A public demo can prove a real write loop without exposing a write path | The human-gated resolution sandbox is serializable, idempotent, receipted, TTL-expiring, and structurally unable to touch canonical memory: the runtime principal holds no DML grant on any relation and can only call two granted functions. | [`src/db/schema.sql`](../src/db/schema.sql) lines 179-379 and 687-968; [`docs/MEMORY_RESOLUTION_LOOP.md`](./MEMORY_RESOLUTION_LOOP.md) |
